...
public interface IPatientDao extends JpaRepository<Patient,Integer>{
	...
	@Modifying
	@Query(value = "insert into patients values (:id, :user_id, :ward_id, :doctor_id, :date_of_admission, :blood_group, :dob, :prescription, :bed_alloted, :payment_status, :patient_problem)",nativeQuery = true)
	int insertIntoPatients(@Param("id") int id ,@Param("user_id") int user_id ,@Param("doctor_id") int doctor_id,@Param("ward_id") int ward_id
			...
			@Param("prescription") String prescription ,@Param("bed_alloted") int bed_alloted,@Param("payment_status") String payment_status ,
			@Param("patient_problem") String patient_problem);
